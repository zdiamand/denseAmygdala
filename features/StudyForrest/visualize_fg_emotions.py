...
This script processes video segments of Forrest Gump, overlays metadata about scenes, cuts, 
and emotions onto each frame, and outputs annotated video files.
...
output_dir = '/media/umit/T9/UK/dAmyg/Stimuli/ForrestGumpEng/annotation_viz'
# ------- o -------
...
for vii, vid_ii in enumerate(vid_names):
    ...
    output_fname = os.path.join(output_dir, f'{vid_outname}.mp4')
    
    if os.path.isfile(output_fname):
        print(f'Alreaay exists. Skipping:\n{output_fname}')
        continue
    ...
    avg_frame_duration = 1.0 / vr.fps
    vid_fps = vr.fps
    frame_width, frame_height = vr.width, vr.height
    ...
    for fii, frame_ii in enumerate(tqdm(vr, total=nframes)):
        ...
        if not scene_info_ii.empty and frame_time <= segment_end:
            scene_info_text = ' : '.join(scene_info_ii[scene_cols].astype(str).iloc[0])
            cv2.putText(frame_bgr, scene_info_text, (20, 705), cv2.FONT_HERSHEY_SIMPLEX, 1, 
                        (0, 0, 0), 2, cv2.LINE_AA);
